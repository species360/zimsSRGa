...
Sur_main <- function(DataCore,   DeathInformation, 
 ...
 assert_that(is.character(BirthType_sel), length(BirthType_sel) == 1)
  ...
  if (BirthType_sel != "All"){
    Data<- DataCore %>%
      filter(stringr::str_detect(BirthType, pattern = BirthType_sel))
  }else{Data<- DataCore}
...
test_that("Sur_main works", {
  ...
  expect_named(out$from0, c('summary', 'metrics', 'bastaRes', 'DICmods', 'KM_estimator', 'bastatab',  'Sur1', 'Sur5', 'check', 'relex_from0', 'Sur1m'))
  ...
  out <- Sur_main(core, DeathInformation = deathinformation, BirthType_sel = "Captive", 
                  Models = "LO", Shape = "bathtub", MaxNSur = 10,
                  niter = 1000, burnin = 101, thinning = 10, nchain = 3, ncpus = 3)
    ...
    if(summar$lxMin < 0.75){
      ...
      if ("Raw" %in% CalculateMetricsFrom){
        ...
        if(length(Datar$BirthDate[Datar$GlobalStatus == "Alive"])>0){
          ...
          Datamax = Datar%>%filter(year(BirthDate)<maxcoh)
          if(nrow(Datamax)>0){
            metrics = rbind(metrics,
                            tibble(Data = 'Raw_Deadcoh',
                                   firstage = MinAge,
...
Sur_out <- function(out,
  ...
  LT = out$bastaRes$lifeTable$noCov$ple%>%
    mutate(Ages = round(Ages/(5*10^(-roundN)))*(5*10^(-roundN)))%>%
    group_by(Ages)%>%
